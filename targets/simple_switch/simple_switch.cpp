...
public:
    ...
    void pop_back(std::unique_ptr<Packet> *pItem)
    {
        ...
        cvar_can_pop.wait(
            lock, [this]
            { return (queue_hi.size() + queue_lo.size()) > 0; });
...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port,
                           size_t nb_queues_per_port, std::string table_lookup)
    : Switch(enable_swap),
      drop_port(drop_port),
...
void SimpleSwitch::egress_thread(size_t worker_id)
{
    ...
    while (1)
    {
        ...
        if (with_queueing_metadata)
        {
            ...
            phv->get_field("queueing_metadata.deq_qdepth").set(egress_buffers.size(port, priority));
